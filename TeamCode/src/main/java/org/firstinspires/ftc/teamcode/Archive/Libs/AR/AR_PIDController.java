...
public class AR_PIDController
{
    ...
    private double f;
    private double tpd;  // For GoBilda 30 RPM Motor => = 5281.1 / 360
    LinearOpMode bot;
     ...
     */
    public AR_PIDController(LinearOpMode iBot, DcMotor iMotor, String iJointName, double ticksPerDegree, double iP, double iI, double iD, double iF, boolean activateFuzzyLogic) {
        this.bot = iBot;
        this.motor = iMotor;
        ...
        d = iD;
        f = iF;
        this.tpd = ticksPerDegree;
        setFuzzyLogicActive(activateFuzzyLogic);
        controller = new PIDController(p, i, d);
        loopCount = 0; // Set to 0 when initialized
    }
    public AR_PIDController(LinearOpMode iBot, DcMotor iMotor, String iJointName, double ticksPerDegree, double iP, double iI, double iD, double iF) {
        this.bot = iBot;
        this.motor = iMotor;
        this.jointName = iJointName;
        p = iP;
    ...
    { // Input in degrees
        if (fuzzyLogicActive) {
            double error = target - (motor.getCurrentPosition() / this.tpd);  // Current error
        ...
        if (this.jointName.equals("first_joint")) {
            armPos = this.motor.getCurrentPosition() + (AR_Arm_Fisher.FIRST_JOINT_START * this.tpd); // armPos is in Ticks
        }
        else if (this.jointName.equals("second_joint")) {
            armPos = this.motor.getCurrentPosition() + (AR_Arm_Fisher.SECOND_JOINT_START * this.tpd); // armPos is in Ticks
        }

        double pid = this.controller.calculate( armPos, target * this.tpd );  // target is in degrees
        double ff = Math.cos( Math.toRadians( target * this.tpd ) ) * f;  // Feedforward
        double power = pid + ff;  // Total power from PID and feedforward

        if ((this.jointName.equals("first_joint")) && (iLastState == AR_Arm_Fisher.DEPLOY) && ((armPos / this.tpd) < target - 10)) {
        ...
        this.bot.telemetry.addData("Pos", " (" + this.jointName + ") " + armPos / this.tpd);
